...
The demo video shows the system in action: a simulated robot navigating a Isaac sim scene while a furniture point cloud cluster stands across its path that was not previously scanned in the static map. The navmesh tiles around the cluster are being rebaked in real time, and the robot steers around the obstacle with clean, oscillation-free trajectories. That moment, when the first dynamic cylinder appeared in the navmesh and the path rerouted around it, made every hour debugging ctypes segfaults worth it.
